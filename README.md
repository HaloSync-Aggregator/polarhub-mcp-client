...
```
사용자 (브라우저)
    ↓ 자연어 입력: "서울에서 싱가포르 4월 15일 항공편 검색해줘"
    ↓
┌─────────────────────────────────────┐
│  PolarHub MCP Client (이 프로젝트)    │
│  - LLM이 의도를 파악                  │
│  - MCP 서버의 적절한 Tool을 호출       │
│  - 결과를 사용자 친화적으로 요약        │
└─────────────────────────────────────┘
    ↓ MCP Protocol (Streamable HTTP)
┌─────────────────────────────────────┐
│  PolarHub MCP Server (별도 서버)      │
│  - 15개 MCP Tool 제공                │
│  - PolarHub NDC API 연동             │
│  - 항공사별 패턴 자동 처리             │
└─────────────────────────────────────┘
    ↓
  PolarHub NDC API → 40+ 항공사
```
...
│  ┌─────┴──────────────┴────────────┴───────┐ │
│  │           Orchestrator                   │ │
│  │  LLM Intent Parsing → MCP Tool 호출     │ │
│  │  → 결과 요약                             │ │
│  └──────────────────────────────────────────┘ │
└──────────────────────────────────────────────┘
         │                          │
...
| **MCP Client** | `src/mcp/client.ts` | MCP 서버 연결, Tool 호출, HMAC 인증, 재연결 로직 |
...
│   ├── mcp/
│   │   └── client.ts             # MCP 클라이언트 — Streamable HTTP + HMAC + 재연결
│   ├── orchestrator/
...
| `HMAC signature mismatch` | 시크릿 불일치 또는 시간 동기화 문제 | `POLARHUB_API_SECRET` 확인, 서버 시간 동기화 |
