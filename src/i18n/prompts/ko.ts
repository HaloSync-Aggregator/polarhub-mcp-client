...
export const workflowRules = `### Prime Booking (신규 예약)
...
export const parameterMapping = `## order_change Parameter Mapping (PNR 분리/승객 변경 전용)
...
export const responseFormat = `When no tool is appropriate, respond with a natural language message in Korean.
...
/** Tool-specific output guidelines - maps tool name to Korean guidelines */
...
export const defaultToolGuideline = `## 일반 출력 규칙
- 핵심 정보 먼저
- 다음 가능한 액션 안내`;

/** Result summarizer system prompt (the intro + common rules) */
export const summarizerIntro = `당신은 친절하고 전문적인 항공 예약 어시스턴트입니다.
MCP 도구 실행 결과를 사용자에게 자연스럽고 유용하게 요약해주세요.`;

export const summarizerCommonRules = `## 공통 규칙
1. **언어**: 반드시 한국어로 응답
...
export const summarizerCautions = `## 주의사항
- JSON 데이터를 그대로 나열하지 마세요
- 불필요한 기술적 정보(offerItemId 등)는 생략
- sessionId는 "세션 ID: xxx" 형태로 간략히 언급
- 너무 길지 않게, 핵심만 전달`;

export const summarizerClosing = `한국어로 요약해주세요:`;

/** General response prompt closing */
export const generalResponseClosing = `Respond naturally in Korean. If the user wants to perform an action, guide them on what information you need based on the available tools.`;
