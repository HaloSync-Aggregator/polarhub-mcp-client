...
```
User (Browser)
    ↓ Natural language: "Search flights from Seoul to Singapore on April 15"
    ↓
┌─────────────────────────────────────┐
│  PolarHub MCP Client (this project) │
...
┌─────────────────────────────────────┐
│  PolarHub MCP Server (separate)     │
│  - Provides 15 MCP Tools            │
│  - Integrates with PolarHub NDC API  │
│  - Handles airline-specific patterns │
└─────────────────────────────────────┘
    ↓
  PolarHub NDC API → 40+ Airlines
```
...
│  ┌─────┴──────────────┴────────────┴───────┐ │
│  │           Orchestrator                   │ │
│  │  LLM Intent Parsing → MCP Tool call     │ │
│  │  → Summarization                        │ │
│  └──────────────────────────────────────────┘ │
└──────────────────────────────────────────────┘
         │                          │
...
| **MCP Client** | `src/mcp/client.ts` | MCP server connection, tool calls, HMAC auth, reconnection |
...
- **Node.js** >= 18
- **PolarHub MCP Server** running (local or Gateway)
- **LLM API Key** (choose one)
  - OpenAI API Key (`sk-...`)
  - Google Gemini API Key
  - Bedrock API Key (`BEDROCK_API_KEY`, Bearer token auth)

> `.env.example` defaults to **AgentCore Gateway + HMAC enabled**. Only switch `MCP_SERVER_URL` and `MCP_GATEWAY_HMAC_ENABLED` when you are pointing to a local MCP server.
...
# MCP server: defaults to AgentCore Gateway (no change needed)
# MCP_SERVER_URL is already set in .env.example
...
| `POLARHUB_SITE_CODE` | `01` | Site code |
| `POLARHUB_AGENCY_CONTACT_ID` | `CTC_TA1` | Agency contact ID |
| `MCP_GATEWAY_HMAC_ENABLED` | `true` | Gateway HMAC authentication mode |
| `MCP_GATEWAY_AGENCY_ID` | (TENANT_ID) | HMAC agency ID |
...
This client connects to the MCP server via **Streamable HTTP** transport.
PolarHub credentials are passed through as `X-PolarHub-*` HTTP headers to the MCP server.
...
### Default: Gateway (AgentCore)

The defaults in `.env.example` point to the **AgentCore Gateway**. Just set your PolarHub credentials and you're ready to go:
...
In this mode, the client generates an HMAC-SHA512 signature for every request sent to the Gateway.

### Local Development Mode (for MCP server code changes)

Use local mode only when directly modifying/debugging the MCP server:
...
| `POLARHUB_API_SECRET` | HMAC signing secret (Base64) | Request from PolarHub admin |
| `POLARHUB_API_BASE_URL` | API endpoint | Sandbox: `https://polarhub.sandbox.halo-platform.net` |
...
│   ├── mcp/
│   │   └── client.ts             # MCP client — Streamable HTTP + HMAC + reconnection
│   ├── orchestrator/
...
| `HMAC signature mismatch` | Secret mismatch or time sync issue | Verify `POLARHUB_API_SECRET`, sync server clock |
